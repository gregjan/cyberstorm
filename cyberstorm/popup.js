function start() {
	...
	var bookmarkTreeNodes = chrome.bookmarks
			.getSubTree('0', function(bookmarkTreeNodes) {
				for(i=0;i<bookmarkTreeNodes.length;i++){
					    //alert(bookmarkTreeNodes[i].id);
					    //bookmarkTreeNodes[i].name = 'fooo';
					    //alert(bookmarkTreeNodes[i].name);
					    //bookmarkTreeNodes[i].name = bookmarkTreeNodes[i].title;
				}
				var nodes = tree.nodes(bookmarkTreeNodes), links = tree
						.links(nodes);

				var link = svg.selectAll(".link").data(links).enter().append(
						"path").attr("class", "link").attr("d", diagonal);
				var node = svg.selectAll(".node").data(nodes).enter().append(
